...
function scloader(){
	...
	function pageAjaxscjl(id){
		$.ajax({
			type:"get",
			url:'http://www.shoutike.com/selectColl?startpage='+id,
			async:true,
			dataType: 'json',
			success: function(data){
			    /**
...
function pageAjaxbfjl(id){
	$.ajax({
		type:"get",
		url:'http://www.shoutike.com/selectPlay?startpage='+id,
		async:true,
		dataType: 'json',
		success: function(data){
		    /**
...
function fromplay(obj){
	...
	var uid=$("#uid").val();
	 $.ajax({  
		   url:"http://www.shoutike.com/selectID?sid="+SID,
           type: "POST",  
           success:function(data){  
        	   //alert(data);
        	   if(data==true){
        		   $.post("http://www.shoutike.com/selectHelpCode?uid="+uid,"",function(date){
        			  //alert(date+"444");
        			   if(date==true){
        			    window.location.href = "http://www.shoutike.com/ffpplay?seriesID="+SID+"&id="+id;

	        	   }else{
	                 window.location.href = "http://www.shoutike.com/play?seriesID="+SID+"&id="+id;
	        	   }  
        		   });
        	   }else{
	                 window.location.href = "http://www.shoutike.com/play?seriesID="+SID+"&id="+id;
...
function deleteColl(obj){
	...
	var vid=$(obj).attr("send");
	 $.ajax({  
		 url:"http://www.shoutike.com/deleteCollect?uid="+uid+"&vid="+vid,
           type: "POST",  
           success:function(data){  
        	   getCollect();
        	   scloader();
